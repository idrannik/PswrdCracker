...
#############################
def dictionary_attack(file, wordlist, start): # dictionary attack on provided hash file from -p.  -d
    found = []
    with open(wordlist, "r") as f:
...
def iterated_dictionary_attack(file, wordlist, start): # iterated dictionary attack on provided hash file from -p.  -id
    found = []
    with open (wordlist, "r") as f:
        for i, word in enumerate(f):
            if i < start:
...
def salted_dictionary_attack(file, wordlist, start): # salted dictionary attack on provided hash file from -p.  -sd
    found = []
    with open (wordlist,"r") as f:
        for i, word in enumerate(f):
            if i < start:
...
def brute_force_attack(hash_to_crack, max_length=4): # Brute force attack on provided hash file from -p. -b
...
#############################
if args.cf:
    file = load_hashes(args.p)              # cracking mode, takes results of choosen attack and outputs them to terminal or a file if -w is used as an argument
    result = None
